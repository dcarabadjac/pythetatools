...
analysis_type_to_xtickspos = {'Erec':np.arange(0, 3.5, 0.5), 'e-theta':np.arange(0, 3.5, 0.5), 'p-theta': np.arange(0, 1300, 200),
                              'P':np.arange(0, 1300, 200), 'Theta':np.arange(0, 200, 20)}
...
osc_param_name_to_xlabel = {"delta": {'both': r"$\delta_{CP}$", 0:r"$\delta_{CP}$", 1:r"$\delta_{CP}$"},
                            "dm2":   {'both': r"$\Delta m^2_{32} \mathrm{ (NO)}/|\Delta m^2_{31}| \mathrm{ (IO)}$, $[\mathrm{eV}^2/\mathrm{c}^4]$", 0: r"$\Delta m^2_{32}$", 1:r"$|\Delta m^2_{31}|$"},
...
interaction_suffixes_2 = ['CC_QE', 'CC_MEC', 'CC_1PI', 'CC_OTHER', 'NC'] #after int. modes merging
